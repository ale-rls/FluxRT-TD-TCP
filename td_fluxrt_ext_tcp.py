 ...
 *      original relay's input path. Kept as latestInputJpeg (latest-wins).
 ...
 *
 * Self-throttling: input frames are forwarded on a rate-capped
 * requestAnimationFrame loop that only sends once the socket has drained
 * the previous frame (bufferedAmount === 0), always sending the newest
 * frame — never flood the link, never queue stale frames.
 *
...
// --- Latest input frame received from TD locally ---
